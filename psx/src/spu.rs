...
struct Reverb {
	...
	// FIR filter
	even_tick: bool,
	input_filter: FirFilter,
	output_filter: FirFilter,
...
impl Reverb {
	...
	fn write(&mut self, addr: u32, write: u16) {
		match addr & 0xFFFF {
			...
			// input volume
			0x1DFC => self.vLIN = write as i16 as i32,
			0x1DFE => self.vRIN = write as i16 as i32,
	...
	fn tick(&mut self, sample_l: i32, sample_r: i32, sram: &mut SoundRam) -> (i16, i16) {
		self.input_filter.push_sample(sample_l, sample_r);

		if self.even_tick {
			let resampled_input = self.input_filter.resample();
			let input_l = apply_volume_i32(resampled_input.0, self.vLIN).clamp(-0x8000, 0x7FFF);
			let input_r = apply_volume_i32(resampled_input.1, self.vRIN).clamp(-0x8000, 0x7FFF);
			...
			self.output_filter.push_sample(apf2_l as i32, apf2_r as i32);
		} else {
			self.output_filter.push_sample(0, 0);
		}

		let output = self.output_filter.resample();

		self.even_tick = !self.even_tick;

		(saturate_sample(apply_volume_i32(output.0 << 1, self.volume_l as i32)), saturate_sample(apply_volume_i32(output.1 << 1, self.volume_r as i32)))
	}
	...
	fn reflection_filter(&mut self, sample: i32, m_addr: usize, d_addr: usize, sram: &mut SoundRam) {
		...
		let reflection = apply_volume_i32(d_sample, self.vWALL);
		let write = apply_volume_i32(sample + reflection - m_sample, self.vIIR).wrapping_add(m_sample);
	...
	fn comb_filter(&mut self, m_comb1: usize, m_comb2: usize, m_comb3: usize, m_comb4: usize, sram: &mut SoundRam) -> i32 {
		let comb = (self.read_reverb(m_comb1, sram) * self.vCOMB1)
			+ (self.read_reverb(m_comb2, sram) * self.vCOMB2)
			+ (self.read_reverb(m_comb3, sram) * self.vCOMB3)
			+ (self.read_reverb(m_comb4, sram) * self.vCOMB4);
...
impl FirFilter {
	fn push_sample(&mut self, sample_l: i32, sample_r: i32) {
		self.samples_l[self.index] = sample_l;
		self.samples_r[self.index] = sample_r;
		self.index = (self.index + 1) % 39;
	}

	fn resample(&self) -> (i32, i32) {
		let mut sample_l = 0;
		let mut sample_r: i32 = 0;
		let mut index = self.index;
		
		for filter in REVERB_FIR_TABLE.iter() {
			sample_l += self.samples_l[index] * filter;
			sample_r += self.samples_r[index] * filter;

			index = (index + 1) % 39;
		}

		(sample_l >> 15, sample_r >> 15)
	}
}
